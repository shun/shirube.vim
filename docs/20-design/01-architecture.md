...
1. `:Shirube <dir>` または `shirube://` を検知する。
   - デフォルトでは、現在のタブページ内（または他タブ）で既に開かれている Shirube ウィンドウを探索し、存在すればそこへジャンプして内容を更新する（シングルトン動作）。
   - `:Shirube! <dir>` (bang 付き) で実行した場合は、ウィンドウの再利用を行わず現在のウィンドウで新しく開く。
2. Vimscript が Denops に通知し、Deno 側で Adapter を解決する。
3. Adapter が一覧を返し、Renderer が行/ハイライトを生成する。
4. バッファに描画し、不可視 ID を Conceal する。
5. バッファ設定として `bufhidden=wipe` を適用し、画面から消えた際に自動的にバッファと Deno 側の State を破棄する。
...
- 確認 UI は `confirm_ui_mode` でフローティング/専用バッファを切り替える。
